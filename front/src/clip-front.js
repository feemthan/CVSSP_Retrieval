...
: ''}
  ...
  html`<option value=${modality} ?selected=${modality === this.modality}>${modality}</option>`)}</select><br />
      <label>Image only<input type="checkbox" ?checked="${this.imageonly}" @click=${() => { this.imageonly = !this.imageonly }} /></label><br />
      <label>Text only<input type="checkbox" ?checked="${this.textonly}" @click=${() => { this.textonly = !this.textonly }} /></label><br />
